...
function AppCanvas() {
  ...
  const handleChatModelSelection = (sku) => {
    if (comparisonMode) {
      // If in comparison mode, load the model to the active canvas
      set_glassesModel(sku, activeCanvas === 'right');
    } else {
      // In normal mode, just set the model
      set_glassesModel(sku);
    }
  };
  ...
  return (
    ...
    <div className="vto-container">
      <div className={`main-content ${comparisonMode ? 'comparison-mode' : ''}`}>
          ...
          <div className="models-container">
            {filteredGlasses.map((model) => (
              <div key={model.id} className="model-item">
                <button
                  className={`model-button ${comparisonMode
                      ? comparedGlasses.includes(model)
                        ? 'active'
                        : ''
                      : currentModel === model.sku
                        ? 'active'
                        : ''
                    }`}
                  onClick={() =>
        ...
        <ChatWidget 
          onSelectModel={handleChatModelSelection}
          glassesModels={glassesModels}
          glassCategories={GLASS_CATEGORIES}
        />
      </div>
